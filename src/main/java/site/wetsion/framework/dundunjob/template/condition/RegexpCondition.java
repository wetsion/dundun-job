...
import site.wetsion.framework.elastic.search.assistant.query.RegExp;

@Component
public class RegexpCondition extends Condition {
    @Override
    public ConditionEnum type() {
        return ConditionEnum.REGEXP;
    }

    @Override
    public QueryCondition translate(StandardTemplateItem item) {
        return new RegExp(item.getKey(), CollectionUtils.isEmpty(item.getValues()) ?
